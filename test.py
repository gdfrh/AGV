...
# print(order_matrix)
#
# # 遍历矩阵的 **列**，同步处理所有订单的同一步骤
# print("\n按列遍历（同步处理订单步骤）：")
# for j in range(max_steps):  # 遍历每一列（相同步骤）
#     print(f"\n处理第 {j+1} 个步骤:")
#     for i in range(len(orders)):  # 遍历每个订单（按行取出元素）
#         step = order_matrix[i, j]
#         if step is not None:  # 过滤掉 None 值
#             print(f" 订单 {i+1} 在执行 {step}")
def find_last_none_index(order_matrix, idx):
    # 获取指定行
    row = order_matrix[idx]
    # 从后向前查找 None 的索引
